...
export type DbFolder = z.infer<typeof folderDocumentSchema>;
...
export const SEED_FOLDERS: readonly DbFolder[] = [];
export async function getFoldersCollection(): Promise<Collection<DbFolder>> { return (await getDatabase()).collection<DbFolder>(FOLDERS_COLLECTION_NAME); }
export function getFoldersCollectionFromDatabase(database: Db): Collection<DbFolder> { return database.collection<DbFolder>(FOLDERS_COLLECTION_NAME); }
