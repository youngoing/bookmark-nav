...
export const tagDocumentSchema = z.object({ id: z.string(), ownerId: z.string(), name: z.string(), color: z.string(), parentId: z.string().nullable(), collectionId: z.string().nullable(), createdAt: z.string(), updatedAt: z.string() }).strict();
export type TagDocument = z.infer<typeof tagDocumentSchema>;
export const TAGS_COLLECTION_NAME = "tags" as const;
export const TAGS_VALIDATOR: Document = { $jsonSchema: { bsonType: "object", required: ["id", "ownerId", "name", "color", "parentId", "collectionId", "createdAt", "updatedAt"], additionalProperties: false, properties: { _id: { bsonType: "objectId" }, id: { bsonType: "string" }, ownerId: { bsonType: "string" }, name: { bsonType: "string" }, color: { bsonType: "string", pattern: "^#[0-9a-fA-F]{6}$" }, parentId: { bsonType: ["string", "null"] }, collectionId: { bsonType: ["string", "null"] }, createdAt: { bsonType: "string" }, updatedAt: { bsonType: "string" } } } };
