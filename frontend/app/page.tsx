...
import { Archive, ArrowUpRight, BarChart3, Bookmark, BookOpen, BriefcaseBusiness, Camera, Check, ChevronLeft, ChevronRight, Circle, Clock3, Code2, Compass, Copy, Download, FolderOpen, Gamepad2, Globe2, Heart, Home as HomeIcon, KeyRound, LayoutGrid, Link2, List, LogOut, Menu, Monitor, Moon, Palette, Pencil, PanelsTopLeft, Plus, Search, Settings2, Share2, SlidersHorizontal, Sparkles, Star, Sun, Table2, Tag, Terminal, Trash2, Wrench, X, Zap, type LucideIcon } from "lucide-react";
...
const tagIcons: Record<string, LucideIcon> = { Tag, Code2, BriefcaseBusiness, BookOpen, Camera, Gamepad2, Globe2, Heart, Home: HomeIcon, Palette, Sparkles, Terminal, Wrench, X, Zap };
const TagIcon = ({ icon, ...props }: { icon?: string } & React.ComponentProps<LucideIcon>) => { const Icon = tagIcons[icon || "Tag"] || Tag; return <Icon {...props} />; };
...
export default function Home() {
  ...
  return <div className="shell">
    <aside className={`sidebar ${mobileNav ? "mobile-open" : ""} ${sidebarCollapsed ? "collapsed" : ""}`}>
      <div className="brand"><span className="brand-mark"><Bookmark size={17} strokeWidth={2.6} /></span><span className="brand-name">bookmark-nav</span><button className="icon-button sidebar-collapse" onClick={() => setSidebarCollapsed((current) => !current)} title={sidebarCollapsed ? "展开导航" : "收起导航"} aria-label={sidebarCollapsed ? "展开导航" : "收起导航"}>{sidebarCollapsed ? <PanelsTopLeft size={17} /> : <PanelsTopLeft size={17} />}</button><button className="icon-button sidebar-close" onClick={() => setMobileNav(false)}><X size={18} /></button></div>
      <nav className="nav">
        <p className="nav-label">个人空间</p>
        <button className={`nav-item ${activeView === "all" && activeFolder === "all" && !activeTag ? "selected" : ""}`} title="我的书签" onClick={() => { setActiveView("all"); setActiveFolder("all"); setActiveTag(null); setMobileNav(false); }}><LayoutGrid size={17} /><span className="nav-text">我的书签</span><span className="nav-count">{data.bookmarks.length}</span></button>
        <button className={`nav-item ${activeView === "recent" ? "selected" : ""}`} title="最近添加" onClick={() => { setActiveView("recent"); setActiveFolder("all"); setActiveTag(null); setSort("recent"); setMobileNav(false); }}><Clock3 size={17} /><span className="nav-text">最近添加</span></button>
        <button className={`nav-item ${activeView === "favorites" ? "selected" : ""}`} title="我的收藏" onClick={() => { setActiveView("favorites"); setActiveFolder("all"); setActiveTag(null); setMobileNav(false); }}><Star size={17} /><span className="nav-text">我的收藏</span><span className="nav-count">{data.bookmarks.filter((bookmark) => bookmark.isFavorite).length}</span></button>
        <button className={`nav-item ${activeView === "sites" ? "selected" : ""}`} title="网站" onClick={() => { setActiveView("sites"); setActiveFolder("all"); setActiveTag(null); setMobileNav(false); }}><Globe2 size={17} /><span className="nav-text">网站</span><span className="nav-count">{data.sites.length}</span></button>
        <p className="nav-label folder-label">共享空间</p>
        <button className={`nav-item ${activeView === "discover" ? "selected" : ""}`} title="发现" onClick={() => { setActiveView("discover"); setActiveFolder("all"); setActiveTag(null); setMobileNav(false); }}><Compass size={17} /><span className="nav-text">发现</span></button>
      ...
      <section className="content">
        ...
        {activeView === "discover" ? <>
          ...
          {isBookmarkEditor ? pageLoading ? <LoadingState /> : pageError ? <div className="empty"><Archive size={30} /><strong>书签加载失败</strong><span>{pageError}</span><button className="secondary-button" onClick={() => void loadPage(pageInfo.page)}>重新加载</button></div> : visible.length ? <BookmarkEditMode bookmarks={visible} onEdit={setEditingBookmark} onDelete={(bookmark) => void deleteBookmark(bookmark)} onFavorite={(bookmark) => void toggleFavorite(bookmark)} onShare={(bookmark) => void toggleShare(bookmark)} /> : <div className="empty"><Archive size={30} /><strong>{activeView === "favorites" ? "还没有收藏的书签" : "还没有匹配的书签"}</strong><span>{activeView === "favorites" ? "先回到全部书签收藏内容" : "试试其他关键词或添加一个新书签"}</span><button className="primary-button" onClick={() => openAddBookmark()}><Plus size={16} />添加书签</button></div> : loading || pageLoading ? <LoadingState /> : pageError ? <div className="empty"><Archive size={30} /><strong>书签加载失败</strong><span>{pageError}</span><button className="secondary-button" onClick={() => void loadPage(pageInfo.page)}>重新加载</button></div> : visible.length ? <>
            ...
            {displayMode === "circle" && <BookmarkCircleGrid bookmarks={visible} onClick={clickBookmark} />}
            {displayMode === "compact" && <BookmarkCompactGrid bookmarks={visible} onClick={clickBookmark} />}
...
function TagModal({ tag, tags, onClose, onSaved }: { tag: TagType | null; tags: TagType[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="modal compact-modal" onSubmit={(event) => void submit(event)}><div className="modal-header"><div><p className="eyebrow">标签管理</p><h2>{tag ? "编辑标签" : "创建标签"}</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="关闭"><X size={18} /></button></div><label>名称<input value={name} onChange={(event) => setName(event.target.value)} maxLength={30} required autoFocus /></label><label>父标签<select value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">顶级标签</option>{tags.filter((item) => !item.parentId && item.id !== tag?.id).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>图标<span className="tag-icon-picker">{Object.entries(tagIcons).map(([key, Icon]) => <button type="button" key={key} className={icon === key ? "active" : ""} onClick={() => setIcon(key)} title={key} aria-label={`选择${key}图标`}><Icon size={16} /></button>)}</span></label><label>颜色<div className="color-field"><input type="color" value={color} onChange={(event) => setColor(event.target.value)} aria-label="标签颜色" /><input value={color} onChange={(event) => setColor(event.target.value)} pattern="#[0-9a-fA-F]{6}" required /></div></label>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" disabled={saving}>{saving ? "保存中..." : "保存标签"}</button></div></form></div>;
}
...
function BookmarkCircleGrid({ bookmarks, onClick }: { bookmarks: BookmarkType[]; onClick: (bookmark: BookmarkType) => void }) { return <div className="bookmark-circle-grid" role="list" aria-label="圆圈书签">{bookmarks.map((bookmark) => <a className="bookmark-circle-item" role="listitem" key={bookmark.id} href={bookmark.url} target="_blank" rel="noreferrer" onClick={() => onClick(bookmark)} title={`${bookmark.title} · ${bookmark.domain}`}><span className="circle-favicon"><img src={bookmark.favicon} alt="" /></span><strong>{bookmark.title}</strong><small>{bookmark.domain}</small></a>)}</div>; }

function BookmarkCompactGrid({ bookmarks, onClick }: { bookmarks: BookmarkType[]; onClick: (bookmark: BookmarkType) => void }) { return <div className="bookmark-compact-grid" role="list" aria-label="图标标题书签">{bookmarks.map((bookmark) => <a className="bookmark-compact-item" role="listitem" key={bookmark.id} href={bookmark.url} target="_blank" rel="noreferrer" onClick={() => onClick(bookmark)} title={bookmark.title}><span className="compact-favicon"><img src={bookmark.favicon} alt="" /></span><strong>{bookmark.title}</strong></a>)}</div>; }
