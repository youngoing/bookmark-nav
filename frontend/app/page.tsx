...
function AccountModal({ user, onClose, onSaved }: { user: UserResponse; onClose: () => void; onSaved: (user: UserResponse) => void }) {
  ...
  async function submit(event: React.FormEvent): Promise<void> {
    ...
    const responseResult = await fromPromise(apiFetch("/api/v1/account", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name, currentPassword: passwordChangeRequired ? undefined : currentPassword || undefined, newPassword: newPassword || undefined, confirmPassword: confirmPassword || undefined }) }), () => ({ code: "NETWORK_ERROR", message: "无法连接到服务器" }));
