...
# Deploy the backend build to a server directory and start/restart it with PM2.
#
# Local deploy (default):
#   pnpm deploy:backend
#   TARGET_BACKEND_DIR=/custom/path pnpm deploy:backend
#
# Remote deploy via SSH/SCP:
#   DEPLOY_HOST=1.2.3.4 pnpm deploy:backend
#   DEPLOY_HOST=1.2.3.4 DEPLOY_USER=root DEPLOY_KEY=~/.ssh/id_rsa pnpm deploy:backend
#
...
TARGET_BACKEND_DIR="${TARGET_BACKEND_DIR:-${TARGET_DIR:-/root/server-tools/bookmark-nav-backend-build}}"
TARGET_DIR="$TARGET_BACKEND_DIR"
...
$PNPM_CMD --filter @loomark/backend build
