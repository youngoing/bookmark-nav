...
- `data-theme` 仅保留为旧 CSS 的兼容标记，固定为 `light`；实际界面颜色只能来自 `data-theme-id` 注入的 `--theme-*` 令牌。
- 界面只提供一个主题选择器，第一项固定为“跟随系统”，后续为视觉预设；不再提供独立的白天/黑夜按钮。
...
当前背景使用真实的明日香角色宣传图：
[`Wallpapers.com · Asuka Langley Posing Evangelion`](https://wallpapers.com/png/asuka-langley-posing-evangelion-41mfufzr0pzskgeq.html)。
页面提供署名信息，并标注该图片可免费使用；项目仅保留来源链接，不声明拥有角色或图片版权。若用于公开商业部署，应先核验该页面的授权条款，并建议取得授权后下载到自己的对象存储，避免第三方 CDN 热链和授权风险。
